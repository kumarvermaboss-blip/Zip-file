...
async def download_file(event, msg, user_id, msg_id):
    ...
    file_size = event.file.size if event.file else 0 # NEW
...
async def zipnow(event):
    ...
    messages_to_delete.append(upload_msg.id) # NEW
    ...
    try:
        ...
        await client.send_file(
            ...
            part_size_kb=8192 # 8MB CHUNK = FAST
        )
    ...
    await asyncio.sleep(3) # 3 sec ruko
    try:
        await client.delete_messages(event.chat_id, messages_to_delete) # NEW: Delete progress
    except: pass

    for file in user_files[user_id]:
        if os.path.exists(file): os.remove(file)
    if os.path.exists(zip_name): os.remove(zip_name) # NEW: zip bhi delete
    user_files[user_id] = []
    user_state[user_id] = "idle"
...
async def main():
    ...
    print("✅ Bot Online! v13 RAILWAY ULTRA FAST")
